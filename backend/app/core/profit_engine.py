...
class ProfitEngineBase(ABC):
    @abstractmethod
    def calculate(self, *, market_insight: MarketInsight, supply_offer: SupplyOffer, platform_data: dict | None = None) -> ProfitBreakdown:
...
class ProfitEngine(ProfitEngineBase):
    def calculate(self, *, market_insight: MarketInsight, supply_offer: SupplyOffer, platform_data: dict | None = None) -> ProfitBreakdown:
        ...
        cost_price = round(alibaba_reference_cost, 2)
        shipping_cost = round(6.5 + supply_offer.shipping_days * 0.35, 2)
        platform_fee = round(selling_price * 0.12, 2)
        ad_cost = round(selling_price * 0.08, 2)
        ...
        data_source_types = {item.get('data_source_type', 'mock') for item in [*shopify_candidates, *alibaba_suppliers]} if (shopify_candidates or alibaba_suppliers) else {'mock'}
