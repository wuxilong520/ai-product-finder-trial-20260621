    ...
    <Card className="border-white/8 bg-[#111A2E] p-5">
      <div className="space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-xs uppercase tracking-[0.22em] text-white/40">决策任务</div>
            <div className="mt-2 text-lg font-semibold text-white">任务 #{task.task_id}</div>
          </div>
        ...
        <div className="space-y-2 text-sm text-white/55">
          ...
          <div className="text-white/40">从市场判断到执行建议，整条链路都会保留解释和追踪。</div>
        </div>
        <Button asChild size="sm" className="w-full">
          <Link href={taskDetailRoute(task.task_id)}>进入任务流</Link>
        </Button>
      </div>
    </Card>
  );
